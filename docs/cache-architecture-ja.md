...
クエリ実行
  ...
  │
  ├─ ids あり + meta なし → full scan で応答 + Queue dispatch で全再構築
  │
...
#### 大規模データで ids がボトルネックになる理由

クエリのたびに ids リスト全件を APCu から取得し、PHP の hashmap を生成する:

```php
$ids    = apcu_fetch('kura:products:v1:ids');  // N 件をデシリアライズ
...
- 各サーバーが独立したキャッシュを持つ
- デプロイ後、各サーバーが独立してキャッシュを再構築する（ids が消えた最初のリクエストで自動
...
public function cursor(
    ...
    // meta なし → index 使えない。全再構築 dispatch
    if ($meta === false) {
        $this->dispatchRebuild();
    }

    // meta あり → IndexResolver で絞り込み、meta なし → ids 全件
...
ids、record、meta、index はすべて常に一緒に再書き込みされる。
